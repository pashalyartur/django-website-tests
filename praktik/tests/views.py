...
def teacher(request):
    tests = Test.objects.all()
    return render(request, 'tests/test_list_teacher.html', {'tests': tests})

def infa(request, test_id):
    test = Test.objects.get(pk=test_id)
    results = Result.objects.filter(test=test)
    return render(request, 'tests/infa.html', {'test': test, 'results': results})
...
def generate_test_results_html(request, test_id):
    test = Test.objects.get(pk=test_id)
    results = Result.objects.filter(test=test)
    html = render_to_string('tests/teacher.html', {'test': test, 'results': results})
